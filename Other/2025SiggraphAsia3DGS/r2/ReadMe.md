...
```bash
# Assuming the code has been downloaded, named gs2/
cd gs2
```
...
pip install -v  --no-build-isolation submodules/diff-gaussian-rasterization_fastgs
...
```bash
python full_eval_our.py --ourdata path/to/data --output_path ./results --fast --aa --use_expcomp
