        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    camlist = sampling_cameras(scene.getTrainCameras().copy(), opt.vcd_num_views)
                    importance_score = pruning_score_vcd = None
                    if len(camlist) > 0:
                        importance_score, pruning_score_vcd = compute_gaussian_score_fastgs(
                            camlist,
                            gaussians,
                            pipe,
                            background,
                    ...
                    momentum_add = gaussians.prune_and_densify(
                        ...
                        pruning_score=pruning_score_vcd,
                        importance_threshold=opt.vcd_importance_thresh,
                        pruning_threshold=opt.vcp_prune_thresh,
                        prune_budget=opt.vcp_prune_budget,
                    )
            ...
            # Multi-view consistent pruning pass after densification phase.
            if (
                opt.vcp_interval > 0
                and iteration >= opt.vcp_start_iter
                and iteration <= opt.vcp_end_iter
                and iteration % opt.vcp_interval == 0
            ):
                camlist = sampling_cameras(scene.getTrainCameras().copy(), opt.vcd_num_views)
                if len(camlist) > 0:
